...
권한이 없으면 tkt 가 시작할 때 시스템 다이얼로그를 띄우고 무엇을 해야 하는지 알려준다.
시스템 설정 → 개인정보 보호 및 보안 → 손쉬운 사용에서 쓰는 터미널 앱을 켜면 된다.
권한은 터미널 앱마다 따로이고, 켠 뒤에는 터미널을 껐다 다시 열어야 반영된다.
...
- **카카오톡이 실행 중이고 로그인돼 있어야 한다.** 창은 화면에 보일 필요가 없다 —
  최소화해 Dock 에 내려놔도, `Cmd+H` 로 앱을 숨겨도 읽기·전송 모두 된다.
  카카오톡을 완전히 종료하면 그때는 아무것도 할 수 없다.
- **채팅창이 닫혀 있으면 tkt 가 채팅 목록에서 찾아 연다.** 이때만은 창이 화면에 뜬다.
  한 번 열린 뒤에는 최소화해 두면 다시 튀어나오지 않는다.
